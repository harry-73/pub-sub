...
export function merge(source, target) {
  for (const key of Reflect.ownKeys(source)) { // we want to preserve symbols
    ...
    if (Array.isArray(val)) {
      ...
      for (let i = 0; i < val.length; i++) {
        const item = val[i];
        target[key][i] = isObject(item) ? merge(item, target[key][i] ?? {}) : item;
      }
    } else if (isObject(val)) {
      target[key] = merge(val, target[key] ?? {});
    } else {
      target[key] = val;
    }
  }
...
export const isEqual = (first, second) => {
  ...
  if (first instanceof Set && second instanceof Set) {
    ...
    // Check if all elements in first are also in second
...
export const mergeIntoMinimongo = (data, sub) => {
  ...
  for (const [collectionName, { docs }] of Object.entries(data)) {
    ...
    for (const doc of docs) {
      ...
      localCollection._docs.set(doc._id, finalDoc); // we need to do this so that subs symbol gets added correctly, if we just use .update and .insert, it looks like Meteor clears that symbol
    }
